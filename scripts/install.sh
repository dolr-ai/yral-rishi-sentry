...
for i in 1 2 3 4 5 6; do
  if curl -fsS http://127.0.0.1:9000/_health/ 2>/dev/null | grep -q '"ok"\|ok'; then
    ...
    echo "          createuser --email rishi@gobazzinga.io --superuser"
    echo ""
    echo "  - Make sure Caddy on rishi-1 AND rishi-2 is attached to the"
    ...
    echo "    after every reboot of rishi-1/rishi-2."
    echo ""
    echo "  - If you suspect Caddy has been restarted and sentry.rishi.yral.com"
