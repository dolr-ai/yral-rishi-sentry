...
# because it contains two secrets: the system secret key and the Google OAuth
# client secret. Never commit this file to git — it's created at install time
# on rishi-3 and lives only there.
cat > "${ENV_CUSTOM}" <<EOF
...
# Source of truth for the secret key: this file itself — it's generated
# ONCE on first install and preserved across re-runs.
